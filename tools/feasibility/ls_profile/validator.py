import re
from typing import List, Dict, Any, Tuple
from tools.feasibility.ls_profile.models import ExtractedItem

def safe_number(value, default=0.0):
    try:
        if value is None or str(value).strip() == "" or str(value).lower() == "none":
            return default
        return float(value)
    except (TypeError, ValueError):
        return default

def is_valid_ls_code(code: str) -> bool:
    code = code.strip().upper()
    if not code:
        return False
    if re.match(r"^\d+$", code):
        return False
    if len(code) < 3:
        return False
    if code in ["6KA", "10KA", "40A", "2026", "VND", "3P", "4P", "1P", "2P"]:
        return False
    if code in ["TÊNHÀNG", "MÃHÀNG", "GIÁBÁN"]:
        return False
        
    prefixes = ["ABN", "ABS", "EBS", "EBN", "LA", "LB", "LC", "MC", "MT", "GMP", "BK", "AN", "AS", "TS", "32GR", "32K", "EBE", "UA", "UR", "AR", "MW", "SJ", "SN", "SH", "SR", "SHT", "UVT", "AX", "AL", "MOP", "MI", "IB", "DH", "EH"]
    for p in prefixes:
        if code.startswith(p):
            return True
            
    if re.search(r"[A-Z]", code) and re.search(r"\d", code) and len(code) >= 5:
        return True
    return False

def validate_extracted_item(item: ExtractedItem) -> Tuple[bool, List[str], List[str]]:
    errors = []
    warnings = []
    
    code = item.material_code.strip()
    desc = item.description.strip()
    qty = item.rated_current.strip()
    price = item.unit_price
    item_type = item.type.strip()
    
    # Kiểm tra xem có phải danh sách dòng điện định mức hợp lệ hay không (kết thúc bằng chữ A và chứa dấu phẩy)
    has_ampere_list_in_desc = bool(desc and re.search(r"\b\d{1,3}(?:,\d{1,3})+A\b", desc.upper()))
    is_ampere_list = (qty.strip().upper().endswith("A") and "," in qty) or has_ampere_list_in_desc
    
    # 1. Validate material_code
    if not code:
        errors.append("material_code_empty")
    else:
        if " " in code:
            errors.append("material_code_contains_space")
        if len(code) > 18:
            errors.append("material_code_too_long_merged")
        if not is_valid_ls_code(code):
            errors.append("material_code_invalid_format")
        if code in ["6KA", "10KA", "40A", "2026"]:
            errors.append("material_code_cannot_be_technical_spec_or_year")
            
    # 2. Validate type
    if item_type:
        item_type_upper = item_type.upper().strip()
        if re.match(r"^(ABN|ABS|EBS|EBN|LA|LB|LC|MC|MT)\w{6,}", item_type_upper):
            errors.append("type_cannot_be_material_code")
        if "," in item_type and re.search(r"\d{3}", item_type) and not item_type.strip().upper().endswith("A"):
            errors.append("type_cannot_be_price")
        if re.match(r"^\d+$", item_type_upper.replace(".", "")):
            val = safe_number(item_type_upper.replace(".", ""))
            if val > 1000:
                errors.append("type_cannot_be_price_or_large_number")
                
    # 3. Validate rated_current
    if qty:
        if "," in qty and re.search(r"\d{3}", qty) and "~" not in qty and "-" not in qty and not is_ampere_list:
            errors.append("rated_current_cannot_be_price")
        if code and code in qty:
            errors.append("rated_current_cannot_contain_material_code")
        if "~" not in qty and "-" not in qty and not is_ampere_list:
            if re.search(r"\d{6,}", qty.replace(",", "").replace(".", "")):
                errors.append("rated_current_too_large")
            
    # 4. Validate unit_price
    if price is None:
        errors.append("unit_price_empty")
    else:
        p_val = int(price)
        if p_val <= 0:
            errors.append("unit_price_zero_or_negative")
        if p_val == 2026:
            errors.append("unit_price_is_year_2026")
        price_str = str(p_val)
        if price_str in ["6", "10", "30", "18", "22", "35", "42", "45", "50", "65", "75", "1", "2", "3", "4"]:
            errors.append("unit_price_cannot_be_technical_spec")
            
    # 5. Validate description
    if desc:
        desc_lower = desc.lower()
        if "bảng giá hàng" in desc_lower or "áp dụng từ ngày" in desc_lower or "ghi chú:" in desc_lower:
            errors.append("description_cannot_be_header_or_footer")
            
        # Không báo lỗi nếu mô tả chứa danh sách dòng định mức dính phẩy kết thúc bằng chữ A
        if re.search(r"\b\d{1,3}(?:,\d{3})+(?:\.\d+)?A\b", desc) and not is_ampere_list:
            errors.append("description_contains_price_with_A")
            
        price_matches = re.findall(r"\b\d{1,3}(?:,\d{3})+(?:\.\d+)?\b", desc)
        for pm in price_matches:
            if is_ampere_list:
                continue
            val = int(pm.replace(",", ""))
            if val > 50000:
                errors.append("description_contains_large_price_noise")
                
        if re.search(r"(ABN|ABS|EBS|EBN|LA|LB|LC|MC|MT)\w{8,}", desc):
            errors.append("description_contains_merged_material_code")

    is_valid = len(errors) == 0
    return is_valid, errors, warnings
